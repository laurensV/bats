...
public class BoTRunner implements Serializable {
   ...
   // static final String electionName = "BoT";
	...
	private static final int SIMPLE_ENERGY = 9;
    ...
    public int maxCostATUSpot = 0;
    public int minCostATUSpot = 0;
...
//
    ...
    public void run() {
        ...
        try {
            ...
            } else if (masterImpl == ENERGY) {
            	master = new EnergyMaster(this);
            } else if (masterImpl == SIMPLE_ENERGY) {
            	master = new SimpleEnergyMaster(this);
            }
...
/*
    ...
    public static void main(String[] args) {
        ...
        BoTRunner.path = System.getenv().get("SPL_BATS_HOME");
        if (BoTRunner.path == null) {
            throw new RuntimeException("Env var SPL_BATS_HOME not set!");
        }
        System.out.println("Path to SPL_BATS set as:\t" + BoTRunner.path);

        masterType = args.length < 1 ? 9 : Integer.parseInt(args[0]);
        deadline = args.length < 2 ? 24 : Long.parseLong(args[1]);
        budget = args.length < 3 ? 400 : Long.parseLong(args[2]);
        size = args.length < 4 ? 53 : Integer.parseInt(args[3]); // required for control tests.
