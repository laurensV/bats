...
public class SamplingPhaseMaster extends Master {
	...
	protected SamplingPhaseMaster(BoTRunner bot) throws Exception {
		...
		bot.minCostATUSpot = Integer.MAX_VALUE;
		bot.maxCostATUSpot = 0;
		...
		for (Cluster cluster : clusters) {
			...
			bot.minCostATUSpot = (int) Math.min(bot.minCostATUSpot, cluster.getCost(Const.current));
			bot.maxCostATUSpot += cluster.maxNodes*cluster.getCost(Const.current);
		}
	...
	protected Job handleJobResult(JobResult received, IbisIdentifier from) {
		...
		doneJob.runtimes.put(cluster.alias, new Double(received.getStats().getRuntime()/1000000000)) ;
		doneJob.done = true;
	...
	private Job findNextJob(Cluster cluster, IbisIdentifier from) {
		...
		if(cluster.regressionPoints.size() < bot.noReplicatedJobs) {
			...
			if(totalSamplingSubmittedCounter < bot.noSampleJobs-bot.noReplicatedJobs) {
				...
				System.out.println("findNextJob: no next job");
				...
				if(timeLeftATU > 0) {
					try{
						nextJob = bot.tasks.remove(random.nextInt(bot.tasks.size()));
						System.out.println("Adauga jobul " +  nextJob.jobID + " in extra points al clusterului" + cluster.alias);
	...
	// TODO why not terminate when sayGB?
	private Job sayGB (IbisIdentifier to) {
		...
		try {
			bot.Clusters.get(cluster).terminateNode(workers.get(cluster).get(node).getIbisIdentifier(), myIbis);
		} catch (IOException e) {
	...
	public void run() {
		...
		System.out.println(schedules.size()+".\t" + BminN + "\t" + mooCheapest.costPlan + "\t" + mooCheapest.noATUPlan);
		...
		if(x < 10) {
			...
			System.out.println(schedules.size()+".\t" + BminPlus10 + "\t" + mooCheapestPlus10.costPlan + "\t" + mooCheapestPlus10.noATUPlan);
		...
		if (x < 20) {
			...
			System.out.println(schedules.size()+".\t" + BminPlus20 + "\t" + mooCheapestPlus20.costPlan + "\t" + mooCheapestPlus20.noATUPlan);
		...
		System.out.println(schedules.size()+".\t" + BmakespanMin + "\t" + mooFastest.costPlan + "\t" + mooFastest.noATUPlan);
		...
		Knapsack mooFastestMinus10 = new Knapsack(items.toArray(new Item[0]),
				BmakespanMinMinus10, jobsLeft, bot.minCostATUSpot,
				bot.maxCostATUSpot,(int)bot.timeUnit);
		HashMap<String, Integer> fastestMinus10Sol = mooFastestMinus10.findSol();
		System.out.println(schedules.size()+".\t" + BmakespanMinMinus10 + "\t" + mooFastestMinus10.costPlan + "\t" + mooFastestMinus10.noATUPlan);
		...
		Knapsack mooFastestMinus20 = new Knapsack(items.toArray(new Item[0]),
				(long)BmakespanMinMinus20, jobsLeft, bot.minCostATUSpot,
				bot.maxCostATUSpot,(int)bot.timeUnit);
		...
		System.out.println(schedules.size()+".\t" + BmakespanMinMinus20 + "\t" + mooFastestMinus20.costPlan + "\t" + mooFastestMinus20.noATUPlan);
		...
		//Spot code
		System.out.println("\tSpot planning");
		spotPlanning(Const.current);
		
		//compute maximum bidding price for each cluster
		Cluster mostProfitableOnDemand = selectMostProfitable();
		Cluster cheapest = findCheapest();
		bot.minCostATUSpot = 0;
		bot.maxCostATUSpot = 0;
		for (Cluster cluster : bot.Clusters.values()){
			cluster.computeMaxPrice(cheapest, mostProfitableOnDemand.profitability);
			bot.minCostATUSpot = (int) Math.min(bot.minCostATUSpot, cluster.getCost(Const.max));
			bot.maxCostATUSpot += cluster.maxNodes*cluster.getCost(Const.max);
		}
		//cheapest.computeCostUnit(cheapest, mostProfitableOnDemand.profitability);
		spotPlanning(Const.max);
		
		bot.minCostATUSpot = 0;
		bot.maxCostATUSpot = 0;
		for (Cluster cluster : bot.Clusters.values()){
			bot.minCostATUSpot = (int) Math.min(bot.minCostATUSpot, cluster.getCost(Const.avg));
			bot.maxCostATUSpot += cluster.maxNodes*cluster.getCost(Const.avg);
		}
		spotPlanning(Const.avg);
	...
	private void spotPlanning(int type) {
		Cluster mostProfitable = selectMostProfitable();
		//Cluster cheapest = findCheapest(on);

		System.out.println("Most profitable machine type: " + mostProfitable.alias 
				+ ", cost: " + mostProfitable.getCost(type) + ", Ti: " + mostProfitable.Ti + " minutes");
		
		for (Cluster cluster : bot.Clusters.values()) {
			cluster.computeJobsPerATU();
		}

		System.out.println(" Spot solutions follow ");
		if (type == Const.max) System.out.println("Using max spot price");
		else if (type == Const.avg) System.out.println("Using average price");
		else System.out.println("Using current price");
		System.out.println("\tB\tC\tM");

		int jobsLeft = bot.tasks.size();
		double makespanBmin = Math.ceil((jobsLeft*mostProfitable.Ti)/bot.timeUnit);
		double Bmin = makespanBmin*mostProfitable.getCost(type);
		...
		for(Cluster cluster : bot.Clusters.values()) {
			for(int i=0;i<cluster.maxNodes;i++)		
				items.add(new Item(1/cluster.Ti,
						(int) cluster.getCost(type),
						cluster.alias));
			maxSpeed += (double) (cluster.maxNodes / cluster.Ti);
			costMaxSpeed += cluster.maxNodes * cluster.getCost(type);
		}
		...
		// selectedSchedule = 0;
		Knapsack mooCheapest = new Knapsack(items.toArray(new Item[0]),
				(long)Bmin, jobsLeft, bot.minCostATUSpot,
				bot.maxCostATUSpot,(int)bot.timeUnit);		
		HashMap<String, Integer> cheapestSol = mooCheapest.findSol();
		int aini = 0;
		for (Cluster cluster : bot.Clusters.values()) {
		...
		while(deltaN > 0) {
			...
			mooCheapest = new Knapsack(items.toArray(new Item[0]),
					(long)Math.ceil(Bmin+(double)(x*Bmin/100)), jobsLeft, bot.minCostATUSpot,					
					bot.maxCostATUSpot,(int)bot.timeUnit);			
			cheapestSol = mooCheapest.findSol();
			aini = 0;
			for (Cluster cluster : bot.Clusters.values()) {
		...
		if(x < 10) {
			...
			long BminPlus10 = (long)(Bmin*1.1); 
			Knapsack mooCheapestPlus10 = new Knapsack(items.toArray(new Item[0]),
					BminPlus10 , jobsLeft, bot.minCostATUSpot,
					bot.maxCostATUSpot,(int)bot.timeUnit);
			HashMap<String, Integer> cheapestPlus10Sol = mooCheapestPlus10.findSol();
			System.out.println(schedules.size()+".\t" + BminPlus10 + "\t" + mooCheapestPlus10.costPlan + "\t" + mooCheapestPlus10.noATUPlan);
			schedules.add(new Schedule(BminPlus10, mooCheapestPlus10.costPlan, mooCheapestPlus10.noATUPlan, cheapestPlus10Sol));
		...
		if (x < 20) {
			...
			long BminPlus20 = (long)(Bmin*1.2); 
			Knapsack mooCheapestPlus20 = new Knapsack(items.toArray(new Item[0]),
					BminPlus20 , jobsLeft, bot.minCostATUSpot,
					bot.maxCostATUSpot,(int)bot.timeUnit);
			HashMap<String, Integer> cheapestPlus20Sol = mooCheapestPlus20.findSol();
			System.out.println(schedules.size()+".\t" + BminPlus20 + "\t" + mooCheapestPlus20.costPlan + "\t" + mooCheapestPlus20.noATUPlan);
			schedules.add(new Schedule(BminPlus20, mooCheapestPlus20.costPlan, mooCheapestPlus20.noATUPlan, cheapestPlus20Sol));
		...
		Knapsack mooFastest = new Knapsack(items.toArray(new Item[0]),
				(long)BmakespanMin, jobsLeft, bot.minCostATUSpot,
				bot.maxCostATUSpot,(int)bot.timeUnit);
		HashMap<String, Integer> fastestSol = mooFastest.findSol();
		...
		if(deltaN > 0) {
			...
			for(int i=orderedByJobsPerATU.size()-1; i>=0; i--) {
				if(leftDeltaN > orderedByJobsPerATU.get(i).maxNodes) {
					BdeltaN += (long) orderedByJobsPerATU.get(i).maxNodes*orderedByJobsPerATU.get(i).getCost(type);
					leftDeltaN -= orderedByJobsPerATU.get(i).maxNodes;
				} else {
					BdeltaN += (long) leftDeltaN*orderedByJobsPerATU.get(i).getCost(type);
					leftDeltaN=0;
					break;
				}
			}
		...
		while(deltaN > 0) {
			...
			mooFastestMinus20 = new Knapsack(items.toArray(new Item[0]),
					((long) Math.ceil(BmakespanMin*(0.8+(double)x/100))), jobsLeft, bot.minCostATUSpot,							
					bot.maxCostATUSpot,(int)bot.timeUnit);			
			fastestMinus20Sol = mooFastestMinus20.findSol();
			aini = 0;
			for (Cluster cluster : bot.Clusters.values()) {
	...
	//second argument important just for spot (if spot == true)
	...
	private String findFailedJob(String clusterName, String node) {
		...
		for(Job j : cluster.extraPoints.values()) {
			if ((!j.done) && (j.getNode().compareTo(node)==0)) {
				...
				bot.tasks.add(j);
				//vf daca afiseaza asta pt t1.micro
