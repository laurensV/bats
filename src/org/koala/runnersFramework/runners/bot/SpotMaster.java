...
public class SpotMaster extends Master{
	...
	// jobsDone/totalruntime; used for estimating the time left
	private double va;
	
	double actualConsumedBudget;
	//bidding strategy
	int strategyType;

	String electionName;
	protected SpotMaster(BoTRunner aBot, Schedule selectedSchedule, int strategy) throws Exception {
		super(aBot);
		...
		for (Cluster cluster : clusters) {
			...
			workers.put(cluster.hostname, workersCluster);
			...
			cluster.setBidPrice(strategyType);
			...
			if(( workers.get(cluster).get(node) != null) && (! workers.get(cluster).get(node).isFinished())) {
				for(Job j : bot.Clusters.get(cluster).subsetJobs.values())
					if (j.getNode().compareTo(node)==0) {
						WorkerStats ws = workers.get(cluster).get(j.getNode());
						...
						if (bot.Clusters.get(cluster).terminatedByProvider(ws.getIbisIdentifier())){
							System.out.println("Instance terminated by provider.");
							if (!ws.isUpdateNeeded())
								ws.updateCost(-ws.getLastPrice());
	...
	protected boolean areWeDone() {
		...
		if (jobsDone == totalNumberTasks) {
			...
			try {
				for (Cluster cluster : clusters) {
					Collection<WorkerStats> wss = workers.get(cluster.hostname).values();					
					System.out.println("Cluster " + cluster.alias + " stats =>");
					for (WorkerStats ws : wss) {
						ws.printStats();
						//terminate worker
						terminateWorker(cluster, ws, " the end of execution ");
					}					
				}

				//TODO update source on vm and check if actual consumed budget is correct
				System.out.println("Due amount " + actualConsumedBudget);
	...
	public void run() {
		...
		timeout = Const.timeout; /*(long) (BoTRunner.INITIAL_TIMEOUT_PERCENT * bot.deadline * Const.minuteInMilis);*/
				...
				for(Job j : bot.Clusters.get(cluster).subsetJobs.values())
					if (j.getNode().compareTo(node)==0) {
						...
						System.err.println("Node " + cfe.ibisIdentifier().location().toString() + 
								...
								" ; cost: " 
								+ (Math.ceil((double)workers.get(cluster).get(j.getNode()).getUptime() / Const.minuteInMilis / bot.Clusters.get(cluster).timeUnit)  
										* bot.Clusters.get(cluster).costUnit));
						break;
					}
	...
	protected Job handleJobRequest(IbisIdentifier from) {
		...
		if(reacquiredMachine == null) {
			workers.get(cluster).put(node, new SpotWorkerStats(node, startTime, from));
			...
			if(bot.Clusters.get(cluster).timestamp > bot.noInitialWorkers)
				workers.get(cluster).get(node).setOffset((double)bot.Clusters.get(cluster).timeUnit-((startTime-this.actualStartTime)%(bot.Clusters.get(cluster).timeUnit*Const.minuteInMilis))/Const.minuteInMilis);
			else 
	...
	public void startInitWorkers() {
		...
		for (Cluster c : clusters) {
			System.out.println("Will ask for nodes: " + c.necNodes);
	...
	public void terminateWorker(Cluster cluster, WorkerStats ws, String reason) {
		long crtTime=0;
		long timeLeftATU = cluster.timeUnit*Const.minuteInMilis - ws.getUptime()%(cluster.timeUnit*Const.minuteInMilis) - Const.minuteInMilis;
		TimerTask tt = null;
		if(timeLeftATU <= 0) {
			try {
				timeLeftATU = 0;
	...
	private void decideWithMovingAverage(boolean socketTimeout) {
		...
		double estimatedConsumedBudget = 0;
		actualConsumedBudget = 0;
		int virtualJobsDone = 0;
		double virtualJobsDoneDouble = 0.0;
		double potentialJobsDone = 0.0;
		//recompute cost, time, jobs left
		for (Cluster cluster : clusters){
			for (WorkerStats ws : workers.get(cluster.hostname).values()){
				if(!ws.isFinished()) {		  
					/*worker is idle*/
					...
					double v;
					int tmp;
					// possible number of ATUs paid for - WC
					int noATUsPaidFor;
					
					//System.out.println("lastReconfigTime="+ lastReconfigTime + " ; startTime=" + ws.getStartTime());
					
					if(ws.getStartTime() <  lastReconfigTime) {
						atusBeforeReconfig = (int) (Math.ceil((double)(lastReconfigTime-ws.getStartTime())/Const.minuteInMilis/cluster.timeUnit));						    
					}
					
					//daca este primul job al workerului
					if(ws.getLatestJobStartTime() == 0) {
						v = (double) (ws.getNoJobs()*Const.minuteInNanos)/((double)ws.getRuntime());

						noATUsPaidFor = (int) Math.ceil((double)crtUptime/ Const.minuteInMilis / cluster.timeUnit);
						...
						if(!ws.isMarked()) {
							...
							if((cluster.noATUPlan-(noATUsPaidFor-atusBeforeReconfig)) == 0) {
								...
								cluster.necNodes --;
								long timeLeftATU = cluster.timeUnit*Const.minuteInMilis - crtUptime%(cluster.timeUnit*Const.minuteInMilis);
								tmp1=((double)timeLeftATU/Const.minuteInMilis)*v;
								tmp = (int) Math.floor(((double)timeLeftATU/Const.minuteInMilis)*v);
							...
							/*	System.out.println("(marked now) node " + ws.getIbisIdentifier().location().toString() +
								...
								va += v;					
								long currentPosition = crtUptime%(cluster.timeUnit*Const.minuteInMilis);
								long timeLeftATU = cluster.timeUnit*Const.minuteInMilis - currentPosition;																				
								tmp1= ((double)timeLeftATU/Const.minuteInMilis)*v;
								tmp = (int) Math.floor(((double)timeLeftATU/Const.minuteInMilis)*v);									
								ws.setTimeLeftATU(timeLeftATU);

								ws.setSpeed(v);
								ws.setFuturePoint((double)(cluster.timeUnit*Const.minuteInMilis - currentPosition - (double)Const.minuteInMilis*tmp/v)/Const.minuteInMilis);
						...
						estimatedConsumedBudget += noATUsPaidFor * cluster.getCost(strategyType);
						
						virtualJobsDone += tmp;
						virtualJobsDoneDouble += tmp1;
						potentialJobsDone += pottmp;
					} else {
						//au mai existat alte joburi

						//underExec ++;
						long taui = System.nanoTime() - ws.getLatestJobStartTime();
						double tauiToMin = (double)taui/Const.minuteInNanos;
						double intermET =  cluster.estimateIntermediary(taui) / Const.minuteInNanos;						
						v = (double) (ws.getNoJobs()+1)/(intermET + (double)ws.getRuntime()/Const.minuteInNanos);
						long timeLeftATU;							
						long currentPosition;
						if(!ws.isMarked()) {								
							noATUsPaidFor = (int) Math.ceil(((double)crtUptime / Const.minuteInMilis + intermET - tauiToMin) 
									/ cluster.timeUnit);								
							/*check if i should terminate this ATU*/
							if((cluster.noATUPlan-((int) Math.ceil((double)crtUptime / Const.minuteInMilis / cluster.timeUnit)-atusBeforeReconfig)) == 0) {
								terminateWorker(cluster, ws, " current ATU is last");
								cluster.necNodes --;
								estimatedConsumedBudget +=  (int)(Math.ceil((double)crtUptime / Const.minuteInMilis / cluster.timeUnit)) * cluster.getCost(strategyType);

								if((int)(Math.ceil((double)crtUptime / Const.minuteInMilis / cluster.timeUnit)) < noATUsPaidFor) {
									timeLeftATU = 0;
									noATUsPaidFor = (int) Math.ceil((double)crtUptime / Const.minuteInMilis / cluster.timeUnit);
								} else {
									timeLeftATU = cluster.timeUnit*Const.minuteInMilis - (crtUptime + (long)((intermET-tauiToMin)*Const.minuteInMilis))%(cluster.timeUnit*Const.minuteInMilis);										
								}
								tmp1=((double)timeLeftATU/Const.minuteInMilis)*(double) (ws.getNoJobs()+1)/(intermET + (double)ws.getRuntime()/Const.minuteInNanos);
								tmp = (int) Math.floor(((double)timeLeftATU/Const.minuteInMilis)*(double) (ws.getNoJobs()+1)/(intermET + (double)ws.getRuntime()/Const.minuteInNanos));
								/*System.out.println("(marked now) node " + ws.getIbisIdentifier().location().toString() +
										": currentPaidForATU=" + noATUsPaidFor +
										" ; timeLeftATU=" + timeLeftATU + 
										" ; atusBeforeReconfig=" + atusBeforeReconfig +
								...
								va += v;
								estimatedConsumedBudget += noATUsPaidFor * cluster.getCost(strategyType);
								currentPosition = (crtUptime + (long)((intermET-tauiToMin)*Const.minuteInMilis))%(cluster.timeUnit*Const.minuteInMilis);
								timeLeftATU = cluster.timeUnit*Const.minuteInMilis - currentPosition;
								tmp1=((double)timeLeftATU/Const.minuteInMilis)*v;
								tmp = (int) Math.floor(((double)timeLeftATU/Const.minuteInMilis)*v);
								ws.setTimeLeftATU(timeLeftATU);

								ws.setSpeed(v);
								ws.setFuturePoint((double)(timeLeftATU - (double)Const.minuteInMilis*tmp/v)/Const.minuteInMilis);
								pottmp = (int)Math.floor(((double)(cluster.noATUPlan-(noATUsPaidFor-atusBeforeReconfig))*cluster.timeUnit
										+ws.getFuturePoint())*v);
								ws.setLastEstimatedUptime(noATUsPaidFor*cluster.timeUnit - ws.getFuturePoint());
								/*System.out.println("node " + ws.getIbisIdentifier().location().toString() +
							...
							noATUsPaidFor = (int) Math.ceil((double)crtUptime/ Const.minuteInMilis / cluster.timeUnit);
							estimatedConsumedBudget += noATUsPaidFor * cluster.getCost(strategyType);
							if(noATUsPaidFor < (int) Math.ceil(((double)crtUptime / Const.minuteInMilis + intermET - tauiToMin) 
									/ cluster.timeUnit)) {
								timeLeftATU = 0;
							} else {
								timeLeftATU = cluster.timeUnit*Const.minuteInMilis - (crtUptime + (long)((intermET-tauiToMin)*Const.minuteInMilis))%(cluster.timeUnit*Const.minuteInMilis);
					...
					estimatedConsumedBudget += Math.ceil((double)ws.getUptime() / Const.minuteInMilis / cluster.timeUnit) * cluster.getCost(strategyType);
				}
				
				if (ws.isUpdateNeeded()){
					ws.lastPriceQuery = (ws.lastPriceQuery == 0) ? ws.getStartTime() : System.currentTimeMillis();
					double newCost = cluster.getCostUsingDate(ws.lastPriceQuery, ws.getIbisIdentifier());
					ws.updateCost(newCost);
				}
				actualConsumedBudget += ws.getCost();
			}
		...
		System.out.println("estimated number of jobs left: " + jobsLeft 
				...
				+ " ; estimated consumed budget: " + estimatedConsumedBudget
				+ " ; actual consumed budget: " + actualConsumedBudget);
		...
		if(((jobsLeft>potentialJobsDone)/*(ebLeft > 1.0*(bot.budget-consumedBudget))*/ && bot.allStatsReady)) {				
			if(items.size() > 1) { 
				...
				Knapsack moo = new Knapsack(items.toArray(new Item[0]),
						(long)bot.budget-(long)actualConsumedBudget, jobsLeft, bot.minCostATU,
						bot.maxCostATU,(int)bot.timeUnit);
				System.out.println("budget available: " + (bot.budget-actualConsumedBudget) + 
						...
						if(decreaseJobsLeftDeltaN) {
							...
							if((jobsLeft - schedule.deltaN) > potentialJobsDone) {
								...
								System.out.println("Nothing changed! " +
										...
										"budget available: " + (bot.budget-actualConsumedBudget) + 
										" ; number of jobs to go: " + jobsLeft +
										" ; out of which deltaN: " + schedule.deltaN + 
										" ; minCostATU: " + bot.minCostATU + " ; maxCostATU: " + bot.maxCostATU);
				...
				/*	System.out.println("cluster " + cluster.alias 
					...
					if(Mi.intValue() > cluster.prevNecNodes) {
						if(Mi.intValue() > cluster.crtNodes + cluster.pendingNodes) {
							...
							ArrayList<WorkerStats> orderedByTimeLeftATU = new ArrayList<WorkerStats>(workers.get(cluster.hostname).values());
	...
	private ArrayList<Item> updateClusterStats(boolean debug) {
		...
		for(Cluster cluster : clusters) {
			...
			if(cluster.Ti != 0 ) {
				bot.minCostATU = (int) Math.min(bot.minCostATU, cluster.getCost(strategyType));
				bot.maxCostATU += cluster.maxNodes*cluster.getCost(strategyType);
				for(int i=0;i<cluster.maxNodes;i++) 
					...
					items.add(new Item((double)cluster.timeUnit/(bot.totalNumberTasks*cluster.Ti),
							(double)cluster.getCost(strategyType)/bot.totalNumberTasks,
							cluster.alias));
							*/
				items.add(new Item(1/cluster.Ti,
							(int) cluster.getCost(strategyType),
							cluster.alias));
				/*DEBUG*/
	...
	private void selectTerminatedWorkers(Cluster cluster) {
		...
		ArrayList<WorkerStats> orderedByArrival = new ArrayList(workers.get(cluster.hostname).values());
